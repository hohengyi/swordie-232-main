...
public class Char {
    ...
    public int getRb120() { return rb120; }
    public int getRb200() { return rb200; }
    public int getRb250() { return rb250; }
    public int getRb300() { return rb300; }

    public void addRb120() { rb120++; }
    public void addRb200() { rb200++; }
    public void addRb250() { rb250++; }
    public void addRb300() { rb300++; }

    public void setRb120(int v) { rb120 = v; }
    public void setRb200(int v) { rb200 = v; }
    public void setRb250(int v) { rb250 = v; }
    public void setRb300(int v) { rb300 = v; }


    public int getCharacterRebirthExpBonus() {
        return rb120 * 10
                + rb200 * 20
                + rb250 * 30
                + rb300 * 40;
    ...
    public int getCharacterRebirthDropBonus() {
        return rb120 * 10
                + rb200 * 20
                + rb250 * 30
                + rb300 * 40;
    ...
    public int getCharacterRebirthMesoBonus() {
        return rb120 * 10
                + rb200 * 20
                + rb250 * 30
                + rb300 * 40;
    ...
    public int getCharacterRebirthStatBonus() {
        return rb120 * 5
                + rb200 * 10
                + rb250 * 15
                + rb300 * 20;
    ...
    public int getTotalRebirthExpBonus() {
        return getCharacterRebirthExpBonus()
                + (getAccount() == null ? 0 : getAccount().getAccountRebirthExpBonus());
    }

    public int getTotalRebirthDropBonus() {
        return getCharacterRebirthDropBonus()
                + (getAccount() == null ? 0 : getAccount().getAccountRebirthDropBonus());
    }

    public int getTotalRebirthMesoBonus() {
        return getCharacterRebirthMesoBonus()
                + (getAccount() == null ? 0 : getAccount().getAccountRebirthMesoBonus());
    }

    public int getTotalRebirthStatBonus() {
        return getCharacterRebirthStatBonus()
                + (getAccount() == null ? 0 : getAccount().getAccountRebirthStatBonus());
    }

    public List<BaseStat> getAutoAssignStats() {
        int job = getJob();

        // Xenon special case
        if (JobConstants.isXenon(job)) {
            return List.of(BaseStat.str, BaseStat.dex, BaseStat.luk);
        }

        BaseStat main = JobConstants.getMainStatByJob(job);

        // Safety fallback
        if (main == null) {
            return List.of(BaseStat.dex);
        }
...
//		getField().broadcastPacket(CFamiliar.familiarEnterField(familiarList, false, false));
    ...
    private double getTotalStatAsDouble(BaseStat baseStat) {
        ...
        switch (baseStat) {
            case expR:
                stat += getTotalRebirthExpBonus();
                break;
            case dropR:
                stat += getTotalRebirthDropBonus();
                break;
            case mesoR:
                stat += getTotalRebirthMesoBonus();
                break;
            case reduceCooltimeR:
            case damR:
            case asr:
            case eleAttrResistance:
            case stance:
            case fd:
            case buffTimeR:
            case summonTimeR:
...
//            // =====================
//            // REBIRTH COOLDOWN OVERRIDE
//            // =====================
            int rbCdrR = getTotalRebirthStatBonus();

            if (rbCdrR > 0 && cdInMillis > 0) {
                int rbReduction = (int) (cdInMillis * (rbCdrR / 100D));
                cdInMillis = Math.max(1000, cdInMillis - rbReduction); // hard floor 1s
            }
