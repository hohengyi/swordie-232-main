...
public class AccountDao implements SworDao<Account> {
    ...
    public void saveOrUpdate(User user, Account account, Char onlyChar) {
        ...
        // 🔥 Ensure account RB totals are always correct before saving
        if (account.getCharacters() != null) {
            account.recalcAccountRebirthTotals();
        }
        ...
        long id = DatabaseManager.executeQuery(query,
                ...
                user.getId(),
                account.getTotalRb120(),
                account.getTotalRb200(),
                account.getTotalRb250(),
                account.getTotalRb300(),
                account.getId()
        );
        if (needsId) {
    ...
    public Account fromResultSet(ResultSet resultSet, String alias) {
        ...
        try {
            ...
            account.setTotalRb120(resultSet.getInt("total_rb_120"));
            account.setTotalRb200(resultSet.getInt("total_rb_200"));
            account.setTotalRb250(resultSet.getInt("total_rb_250"));
            account.setTotalRb300(resultSet.getInt("total_rb_300"));
